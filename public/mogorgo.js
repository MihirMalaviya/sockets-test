...
function renderLoop() {
  ...
  for (let id in clientBalls) {
    // clientBalls[id].pos.x
    // clientBalls[id].pos.y

    ctx.beginPath();
    ctx.arc(
      clientBalls[id].pos.x,
      clientBalls[id].pos.y,
      clientBalls[id].comp[0].r,
      0,
      2 * Math.PI
    ); // Draw a full circle
    ctx.fillStyle = "rgb(124, 93, 79)"; // Set the fill color of the circle
    ctx.fill(); // Fill the circle with the specified color
    ctx.lineWidth = 5;
    ctx.strokeStyle = "rgb(53, 53, 77)";
    ctx.stroke();
    ctx.closePath();
  }
  userInterface();
